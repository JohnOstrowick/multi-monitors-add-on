...
class MultiMonitorsControlsManager extends St.Widget {
    _init(index) {
        ...
        this._layoutManager = new St.Widget({ visible: false, x_expand: true, y_expand: true, clip_to_allocation: true });
        this._pageChangedId = Main.layoutManager.connect('page-changed', this._setVisibility.bind(this));
        this._pageEmptyId = Main.layoutManager.connect('page-empty', this._onPageEmpty.bind(this));

        this._group = new St.BoxLayout({ name: 'mm-overview-group-'+index,
                                         x_expand: true, y_expand: true });
        this.add_actor(this._group);

        this._group.add_child(this._layoutManager);
    ...
    _onDestroy() {
        Main.layoutManager.disconnect(this._pageChangedId);
        Main.layoutManager.disconnect(this._pageEmptyId);
    ...
    getWorkspacesActualGeometry() {
        let geometry;
        if (this._visible) {
            const [x, y] = this._layoutManager.get_transformed_position();
            const width = this._layoutManager.allocation.get_width();
            const height = this._layoutManager.allocation.get_height();
    ...
    _setVisibility() {
        ...
        if (!Main.overview.visible ||
            ...
            let activeWorkspace = Main.layoutManager.getWorkspaceByIndex(Main.layoutManager.getActiveWorkspaceIndex());
            let thumbnailsVisible = activeWorkspace !== null; // This checks if there is an active workspace
    ...
    show() {
        this._layoutManager.visible = true;
        this._workspacesViews = Main.layoutManager._workspacesDisplay._workspacesViews[this._monitorIndex];
        this._visible = true;
    ...
    hide() {
        ...
        this._workspacesViews.ease({
            ...
            mode: Clutter.AnimationMode.EASE_OUT_QUAD,
            onComplete: () => {
                this._layoutManager.visible = false;
            },
        });
        this._workspacesViews = null;
    }
